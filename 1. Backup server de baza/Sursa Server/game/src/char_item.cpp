...
{
	m_dwRefineNPCVID = ch ? ch->GetVID() : 0;
}
...
bool CHARACTER::DoRefine(LPITEM item, bool bMoneyOnly)
{
	...
	{
		const std::unordered_set<LPITEM> skipList = {item}; // @fixme346
		for (int i = 0; i < prt->material_count; ++i)
		{
			if (CountSpecifyItem(prt->materials[i].vnum, &skipList) < prt->materials[i].count) // @fixme346
			{
				if (test_server)
				{
					ChatPacket(CHAT_TYPE_INFO, "Find %d, count %d, require %d", prt->materials[i].vnum, CountSpecifyItem(prt->materials[i].vnum, &skipList), prt->materials[i].count);
				}
		...
		for (int i = 0; i < prt->material_count; ++i)
			RemoveSpecifyItem(prt->materials[i].vnum, prt->materials[i].count, &skipList); // @fixme346
	}
...
bool CHARACTER::DoRefineWithScroll(LPITEM item)
{
	...
	const std::unordered_set<LPITEM> skipList = {pkItemScroll, item}; // @fixme346
	for (int i = 0; i < prt->material_count; ++i)
	{
		if (CountSpecifyItem(prt->materials[i].vnum, &skipList) < prt->materials[i].count) // @fixme346
		{
...
LPITEM CHARACTER::FindItemByID(DWORD id) const
{
	...
	{
		auto pkItem = GetInventoryItem(i);
		if (pkItem && pkItem->GetID() == id)
			return pkItem;
	}

	for (int i=0 ; i < DRAGON_SOUL_INVENTORY_MAX_NUM ; ++i)
	{
		auto pkItem = GetDragonSoulInventoryItem(i);
		if (pkItem && pkItem->GetID() == id)
			return pkItem;
	}
...
int CHARACTER::CountSpecifyItem(DWORD vnum, const std::unordered_set<LPITEM> * skipList) const
{
	int	count = 0;
	LPITEM item;
	...
	for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
	{
		...
		if (NULL != item && item->GetVnum() == vnum)
		{
			...
			if (skipList && skipList->contains(item)) // @fixme346
				continue;
...
void CHARACTER::RemoveSpecifyItem(DWORD vnum, DWORD count, const std::unordered_set<LPITEM> * skipList)
{
	if (0 == count)
		return;
	...
	for (UINT i = 0; i < INVENTORY_MAX_NUM; ++i)
	{
		...
		if (pkItem->GetVnum() != vnum)
			continue;

		if (skipList && skipList->contains(pkItem)) // @fixme346
			continue;

		if(m_pkMyShop)
		{
			bool isItemSelling = m_pkMyShop->IsSellingItem(pkItem->GetID());
			if (isItemSelling)
				continue;
		}

		if (vnum >= 80003 && vnum <= 80007)
			LogManager::instance().GoldBarLog(GetPlayerID(), pkItem->GetID(), QUEST, "RemoveSpecifyItem");

		if (count >= pkItem->GetCount())
		{
			count -= pkItem->GetCount();
			pkItem->SetCount(0);
